...
type TShellFunctions=class
      procedure RegExpDebugOutputHook(const Instance:TBESEN;const Data:TBESENUTF8STRING;NewLine:TBESENBOOLEAN);
...
begin
 ...
 if CountArguments=0 then begin
  ...
  fFileStream:=TFileStream.Create(BESENEncodeString(BESENUTF16ToUTF8(fFileName),UTF_8,BESENLocaleCharset),Mode);
 end;
 s:='';
end;
...
procedure TShellFunctions.RegExpDebugOutputHook(const Instance:TBESEN;const Data:TBESENUTF8STRING;NewLine:TBESENBOOLEAN);
begin
...
var i:integer;
 ...
 begin
  fOutput:=fOutput+BESENUTF16ToUTF8(s);
 end;
begin
 fOutput:='';
 ResultValue.ValueType:=bvtUNDEFINED;
 for i:=0 to CountArguments-1 do begin
  ...
  case v^.ValueType of
   ...
   bvtOBJECT:begin
    writeit(BESENEncodeString(BESENUTF16ToUTF8(TBESEN(Instance).ToStr(v^)),UTF_8,BESENLocaleCharset));
   end;
   bvtREFERENCE:begin
    writeit('reference');
   end;
...
begin
 ...
 for i:=0 to CountArguments-1 do begin
  ...
  case v^.ValueType of
   ...
   bvtSTRING:begin
    writeit(BESENEncodeString(BESENUTF16ToUTF8(v^.Str),UTF_8,BESENLocaleCharset));
   end;
   bvtOBJECT:begin
    writeit(BESENEncodeString(BESENUTF16ToUTF8(TBESEN(Instance).ToStr(v^)),UTF_8,BESENLocaleCharset));
   end;
...
begin
 ...
 try
  if CountArguments>0 then begin
   Content:='[';
   FileName:=BESENEncodeString(BESENUTF16ToUTF8(TBESEN(Instance).ToStr(Arguments^[0]^)),UTF_8,BESENLocaleCharset);
   if FindFirst(FileName,faAnyFile or faDirectory,SearchRec)=0 then begin
    Count:=0;
    repeat
     ...
     Content:=Content+'{';
     Content:=Content+'"name":'+BESENJSONStringQuote(BESENUTF8ToUTF16(BESENEncodeString(SearchRec.Name,BESENLocaleCharset,UTF_8)))+',';
     Content:=Content+'"size":'+IntToStr(SearchRec.Size)+',';
     Content:=Content+'"time":'+BESENFloatToStr(BESENDateTimeToBESENDate(FileDateToDateTime(SearchRec.Time)))+',';
 ...
 try
  if CountArguments>0 then begin
   ...
   if ioresult=0 then begin
    ...
    ResultValue:=TBESEN(Instance).Execute(BESENUTF8ToUTF16(BESENConvertToUTF8(Content)));
    {$i-}closefile(f);{$i+}
    if ioresult=0 then begin
    end;
    filemode:=fm;
...
begin
 ResultValue.ValueType:=bvtSTRING;
 ResultValue.Str:='BESEN Shell v'+BESENVersion+' - Copyright (C) 2010, Benjamin ''BeRo'' Rosseaux';
end;
    ...
    case s[1] of
     '-','+','/':begin
      ...
      if (s='?') or (s='h') or (s='help') then begin
       writeln('BESEN Shell v'+BESENVersion+' - Copyright (C) 2010, Benjamin ''BeRo'' Rosseaux');
 ...
 try
  try
   if not DoExit then begin
    TBESEN(Instance).InjectObject('console',BESENConvertToUTF8(BESENObjectConsoleSource));
   end;
  except
  ...
  if length(FileName)>0 then begin
   try
    if not DoExit then begin
     v:=TBESEN(Instance).Execute(BESENConvertToUTF8(BESENGetFileContent(FileName)));
     Print(v);
    end;
   except
    on e:EBESENError do begin
   ...
   while not DoExit do begin
    ...
    readln(s);
    try
     v:=TBESEN(Instance).Execute(BESENConvertToUTF8(s));
     if not DoExit then begin
      Print(v);
      v.ValueType:=bvtUNDEFINED;
      v.Obj:=nil;
